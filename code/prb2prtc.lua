...
function main()
  ...
  print('Padding Train Dataset')
  pad_train=nn.SpatialZeroPadding(2,2,2,2)
  trainset.data=pad_train:forward(trainset.data)
  image.save('Train_Patterns_P2C.png',image.toDisplayTensor(trainset.data:narrow(1,1,100),1,10))

  print('Padding and Translating Test Dataset')
  pad_test =nn.SpatialZeroPadding(2,2,2,2)
  testset.data=pad_test:forward(testset.data)
  for i = 1,60000 do
    r1 = torch.random(0,2)
    r2 = torch.random(0,2)
    trainset.data[i]=image.translate(trainset.data[i],2,2)
  end

  for i = 1,10000 do
    r1 = torch.random(0,2)
    r2 = torch.random(0,2)
    testset.data[i]=image.translate(testset.data[i],2,2)
  end
  image.save('Test_Patterns_P2C.png',image.toDisplayTensor(testset.data:narrow(1,1,100),1,10))
  end
...
--------------------------------------------------------------------
    ...
    local loss   = criterion:forward(output,target)
    -- Collect Statistics
    confusion_test:batchAdd(output,target)
